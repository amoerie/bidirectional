...
using Serilog;
using Serilog.Core;
using SignalR.Demo.Server.DependencyInjection;
...
namespace SignalR.Demo.Server
{
    public sealed class Program
    {
        public static void Main(string[] args)
        {
            ...
            try
            {
                Log.ForContext<Program>().Information("Starting up SignalR Demo Server");

                CreateHostBuilder(args).Build().Run();
            }
            catch (Exception ex)
            {
                Log.ForContext<Program>().Fatal(ex, "Starting up failed for SignalR Demo Server");
                throw;
            }
            finally
            {
        ...
        private static Logger CreateLogger(string[] args)
        {
            ...
            return new LoggerConfiguration()
                .WriteTo.Async(asyncConfig =>
                {
                    ...
                    asyncConfig.File(
                        path: Path.Join(basePath, "logs\\SignalR.Demo.Server.-.log"),
