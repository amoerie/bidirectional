...
using Bidirectional.Demo.Server.DependencyInjection;
...
namespace Bidirectional.Demo.Server
{
    public sealed class Program
    {
        public static void Main(string[] args)
        {
            ...
            try
            {
                Log.ForContext<Program>().Information("Starting up Bidirectional Demo Server");

                CreateHostBuilder(args).Build().Run();
            }
            catch (Exception ex)
            {
                Log.ForContext<Program>().Fatal(ex, "Starting up failed for Bidirectional Demo Server");
                throw;
            }
            finally
            {
