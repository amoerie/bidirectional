...
using Serilog;
using Serilog.Core;
using SignalR.Demo.Client.DependencyInjection;
...
namespace SignalR.Demo.Client
{
    public sealed class Program
    {
        public static void Main(string[] args)
        {
            ...
            try
            {
                Log.ForContext<Program>().Information("Starting up SignalR Demo Client");

                CreateHostBuilder(args).Build().Run();
            }
            catch (Exception ex)
            {
                Log.ForContext<Program>().Fatal(ex, "Starting up failed for SignalR Demo Client");
                throw;
            }
            finally
            {
        ...
        private static Logger CreateLogger(string[] args)
        {
            ...
            return new LoggerConfiguration()
                .WriteTo.Async(asyncConfig =>
                {
                    ...
                    asyncConfig.File(
                        path: Path.Join(basePath, "logs\\SignalR.Demo.Client.-.log"),
